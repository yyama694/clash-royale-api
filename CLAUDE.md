...
- **フェーズ1(最小機能、実装済み)**: 画面ごとの詳細は`画面一覧.md`を参照。
  - プレイヤータグ検索: 連勝・連敗、ランク戦の順位、使用中のデッキ(ゲーム内で今セットしているもの)、戦績サマリー・得意/苦手カード・直近対戦履歴、対戦詳細(両者のデッキ・タワーユニット)。デッキには平均エリクサー・4枚サイクルとゲームへのコピーボタン
  - カード一覧画面(`/cards`、ヘッダーのメニューからリンク): レアリティ別・エリクサー順。日本語名/英語名(ひらがな可)で絞り込み
