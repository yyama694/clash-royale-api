...
- **フェーズ1(最小機能、実装済み)**: 画面ごとの詳細は`画面一覧.md`を参照。
  ...
  - 個人ランキング: トップページに上位100人、個人ランキング画面(`/ranking/players`)に上位1000人(ランク戦の現在シーズン。ランク戦は2025年6月に公式がパス・オブ・レジェンドから改名したもの。1000人は公式APIが返せる上限)。どちらもグローバル/国・地域別をタブで切替
...
- **トップページはできるだけ軽くする**(2026-09-19にユーザーが表明。大前提の基本方針)。トップページに機能・表示件数を足す判断をするときは、常にこの方針を優先する。低スペックVM(Always Free枠)で動かしている事情とも合致する([[project-oci-vm-spec]]参照)。
  - **未解決の食い違い(2026-09-19に判明)**: この方針は「個人ランキングをトップページで10人に絞った判断」(下記「画面デザインの改善候補」参照)を根拠に挙げているが、実際のコード(`HomeController.PLAYER_RANKING_SIZE`)は前日の`4b13272`(ユーザー判断)で100人に増やされたまま戻っていない。トップページのランキングは`lazyUrl`無し(両タブ即描画)のため、実際には200行(100人×2タブ)を毎回描画している。次にトップページを触るときに、10人に戻すか、100人のまま(方針の記述を直す)か判断すること。
