...
- **トップページはできるだけ軽くする**(2026-09-19にユーザーが表明。大前提の基本方針)。トップページに機能・表示件数を足す判断をするときは、常にこの方針を優先する。低スペックVM(Always Free枠)で動かしている事情とも合致する([[project-oci-vm-spec]]参照)。個人ランキングをトップページで10人に絞った判断(下記「画面デザインの改善候補」参照)も同じ方針に基づく。
