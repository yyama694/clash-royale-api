...
- **Chromeの操作は別セッションと共有している前提で行う(2026-09-24にユーザーが指示)**: Claude in Chromeで操作するChromeは1つだけで、別のセッションも同時に同じChrome(同じXアカウントのログイン状態を含む)を操作していることがある。
  - 自分のタブは`tabs_context_mcp`・`tabs_create_mcp`で自分のセッション用に作ったものだけを使う。他のタブの切り替え・遷移・入力・クローズはしない。以前のセッションのタブIDも使い回さない。
  - クリックや文字入力の前には必ずスクリーンショットで今の画面を確かめる。想定と違う画面(知らない下書きが入った投稿ダイアログ、見覚えのないページなど)のときは、そこで操作をやめてユーザーに確認する。別セッションが作業している途中の可能性があるため。
  - Xへの投稿は、完了表示を見落としても押し直さない。先にプロフィールで投稿されたか確かめてから判断する(二重投稿を防ぐため。別セッションの投稿と取り違えないよう、本文で照合する)。
  - 作業が終わったら、自分が作ったタブは閉じる。
...
- **アクセス計測(2026-09-22に追加)**: 実ブラウザで表示された回数は`/beacon`(`BeaconController`)で数える。ページのJSが`navigator.sendBeacon`で1回だけ叩くため、HTMLを取得するだけのクローラーは現れない。**記録先は専用のストレージを持たずApacheのアクセスログに任せ**、集計は`access-check`スキルが行う(アプリ側は状態を持たない)。識別子もCookieも使わないのでCookie同意の対象外。アクセス数を見るときは`access-check`スキルを使い、**ログのUA/IPからの推定よりビーコンの数字を優先する**(ただしJSを切っている閲覧者は数えられないので下限として読む)。経緯は`進捗ログ.md`のフェーズ1.61。
